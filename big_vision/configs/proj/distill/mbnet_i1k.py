...
def get_config(arg=None):
  ...
  config.input.batch_size = 4096 // 16
  ...
  config.input.pp = (
      f'decode_jpeg_and_inception_crop({teacher_lres})|flip_lr' +
      pp_common.format(lbl='label')+
      f'|copy("image", "{config.teachers[0]}")|resize_small({config.student_res})'
  )
  ppv_student = f'decode|resize_small({teacher_hres})|central_crop({teacher_lres})' + pp_common + f'|resize_small({config.student_res})'

  ppv_teacher = f'decode|resize_small({teacher_hres})|central_crop({teacher_lres})'+ pp_common
  ...
  config.lr = (0.03 * config.input.batch_size) / 4096
  config.wd = (0.0003 * config.input.batch_size) / 4096
  ...
  def get_eval(split, dataset='imagenet2012'):
    return dict(
        ...
        pp_fn=ppv_student.format(lbl='label'),
  ...
  config.evals.student_real.pp_fn = ppv_student.format(lbl='real_label')
  ...
  teacher_eval = dict(
      ...
      pred='prof_m_fwd',
      pp_fn=ppv_teacher.format(lbl='label'),
  )
  ...
  config.evals.teacher_real.pp_fn = ppv_teacher.format(lbl='real_label')
