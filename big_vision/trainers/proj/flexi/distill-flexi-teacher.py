...
def main(argv):
  ...
  # 1. One the student
  student_pfns = {}
  def fwd(params, image, n="student", m=models["student"]):
    return m.apply(params[n], image, train=False)
  student_pfns["student_fwd"] = fwd
  ...
  def predict_fn(params, *, name, **kw):
    ...
    for k in ("student", *config.teachers):
      kw.pop(k, 0)
    return models[name].apply(params[name], image, **kw)
  # 2. One per teacher variant
  teacher_pfns = flexi.mkpredictfns(functools.partial(predict_fn, name="prof_m"),
                                    config.flexi, "prof_m_{x}")
  ...
  for step, batch in zip(range(first_step + 1, total_steps + 1), train_iter):
    ...
    # ]
    flexi_args = [
        flexi.step(config.flexi[n].v, config.flexi[n].steps, step)
        for n in flexi_argnames
    ]
    ...
    for (name, evaluator, log_steps, prefix) in evaluators():
      if u.itstime(step, log_steps, total_steps, last=False):
        write_note(f"Teacher seqhw: {flexi_args[0]}")
